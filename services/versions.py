...
def capture_state():
    ...
    try:
        from ..models.models import Database, Provider as DBProvider, APIKey, AIModel, ModelFallback, EmailAccount
    ...
    emails = []
    for e in EmailAccount(db).get_all():
        emails.append({
            'id': e['id'],
            'email': e['email'],
            'password': e.get('password', ''),
            'email_type': e.get('email_type', 'other'),
...
def apply_state(state):
    ...
    db.conn.execute('DELETE FROM email_account')
    ...
    for e in state.get('emails', []):
        db.conn.execute(
            'INSERT INTO email_account (id, email, password, email_type, created_at) VALUES (?, ?, ?, ?, ?)',
            (e['id'], e['email'], e.get('password', ''), e.get('email_type', 'other'), e.get('created_at', ''))
        )
    ...
    for k in state.get('keys', []):
        email_id = k.get('email_id')
        if email_id is None:
            email_name = k.get('key_name', 'unknown@example.com').strip() or 'unknown@example.com'
            if email_name not in email_cache:
                row = db.conn.execute("SELECT id FROM email_account WHERE email = ?", (email_name,)).fetchone()
                if row:
                    email_cache[email_name] = row[0]
                else:
                    cur = db.conn.cursor()
                    cur.execute("INSERT INTO email_account (email, password, email_type) VALUES (?, ?, ?)", (email_name, "", "other"))
                    email_cache[email_name] = cur.lastrowid
            email_id = email_cache[email_name]

        db.conn.execute(
            '''INSERT INTO api_key (id, provider_id, email_id, key_name, key_value, is_active)
               VALUES (?, ?, ?, ?, ?, ?)''',
            (k['id'], k['provider_id'], email_id, k['key_name'], k['key_value'], k.get('is_active', 1))
        )
    ...
    for table in ('provider', 'model', 'api_key', 'model_fallback', 'email_account'):
