...
def _normalize_model_name(actual_model, provider_name):
    ...
    if normalized_provider:
        if normalized_provider == 'openrouter' and model_name.lower().startswith('openrouter/'):
            return model_name
...
def generate_config(
    ...
    if include_litellm:
        ...
        token = os.environ.get('LITELLM_HELPER_PASSWORD', '')
        port = os.environ.get('PORT', '5001')
        endpoint = f"http://litellm-helper:{port}/api/usage/webhook?token={token}"
    ...
    ALLOWED_LITELLM_PARAMS = {
        ...
        'rpm', 'tpm', 'rpd', 'tpd', 'timeout', 'stream_timeout', 'max_retries',
        'model_info', 'organization', 'api_version', 'drop_params',
        'stop', 'temperature', 'safe_prompt', 'top_p', 'presence_penalty', 'frequency_penalty', 'metadata'
    }
    ...
    def _build_model_entry(model, provider, key, name, is_hermes=False):
        normalized_provider = _normalize_provider_for_litellm(provider.get('provider_type') or model.get('provider_name'))
        normalized_model = _normalize_model_name(model.get('actual_model', ''), provider.get('provider_type') or model.get('provider_name'))
        ...
        hashed_key = __import__('hashlib').sha256(key['key_value'].encode()).hexdigest()
        
        model_info_dict = {
            'id': hashed_key,
            'helper_hashed_key': hashed_key,
            'mode': mode,
        ...
        if normalized_provider in ('gemini', 'groq', 'cerebras', 'vertex_ai', 'vertexai'):
            entry['model_info']['supports_assistant_prefill'] = False
        ...
        if model.get('max_input_tokens') and model['max_input_tokens'] > 0:
            entry['model_info']['max_input_tokens'] = model['max_input_tokens']
