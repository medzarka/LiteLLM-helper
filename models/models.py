...
class Database:
    ...
    def _ensure_model_columns(self):
        ...
        for col, ctype in (('last_checked', 'TEXT'), ('last_status', 'TEXT'), ('last_message', 'TEXT'), ('model_size_b', 'INTEGER'), ('stream_timeout', 'REAL'), ('max_input_tokens', 'INTEGER')):
...
class Provider:
    ...
    def create(self, name, api_base='', description=''):
        cursor = self.db.conn.cursor()
        try:
            cursor.execute(
                'INSERT INTO provider (name, api_base, description) VALUES (?, ?, ?)',
                (name, api_base, description)
            )
            self.db.conn.commit()
    ...
    def get_by_name(self, name):
        cursor = self.db.conn.cursor()
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_provider_name ON provider(name)')
        cursor.execute('SELECT * FROM provider WHERE name = ?', (name,))
        row = cursor.fetchone()
        if row:
            return {
                'id': row[0],
    ...
    def get_by_id(self, provider_id):
        ...
        cursor.execute('SELECT * FROM provider WHERE id = ?', (provider_id,))
        row = cursor.fetchone()
        if row:
            return {
                'id': row[0],
    ...
    def update(self, provider_id, name=None, api_base=None, description=None):
        cursor = self.db.conn.cursor()
        updates = []
        params = []
        if name is not None:
...
class APIKey:
    ...
    def create(self, provider_id, key_name, key_value, active=True):
        cursor = self.db.conn.cursor()
        try:
            cursor.execute(
                'INSERT INTO api_key (provider_id, key_name, key_value, is_active) VALUES (?, ?, ?, ?)',
                (provider_id, key_name, key_value, active)
            )
            self.db.conn.commit()
            return cursor.lastrowid
        except sqlite3.Error as e:
    ...
    def get_by_provider(self, provider_id):
        ...
        cursor.execute(
            'SELECT id, provider_id, key_name, key_value, is_active, created_at FROM api_key WHERE provider_id = ?',
            (provider_id,)
        )
        keys = []
        for row in cursor.fetchall():
    ...
    def update(self, key_id, is_active=None):
        cursor = self.db.conn.cursor()
        if is_active is not None:
            cursor.execute(
                'UPDATE api_key SET is_active = ? WHERE id = ?',
                (is_active, key_id)
            )
            self.db.conn.commit()
            return cursor.rowcount > 0
...
class AIModel:
    ...
    def create(self, provider_name, name, actual_model, model_type='', rpm_limit=30, tpm_limit=6000,
             ...
             supports_function_calling=True, skills=None, model_size_b=None, max_input_tokens=None):
        # Get provider
        cursor = self.db.conn.cursor()
            ...
            cursor.execute(
                '''INSERT INTO model
                   (provider_id, name, actual_model, model_type, rpm_limit, tpm_limit, rpd_limit, tpd_limit,
                    rpm_month_limit, tpm_month_limit, timeout, max_retries, supports_function_calling, skills, model_size_b, max_input_tokens)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                (provider_id, name, actual_model, model_type, rpm_limit, tpm_limit, rpd_limit, tpd_limit,
                 rpm_month_limit, tpm_month_limit, timeout, max_retries, supports_function_calling, skills_json, model_size_b, max_input_tokens)
            )
            self.db.conn.commit()
            return cursor.lastrowid
        except sqlite3.Error as e:
    ...
    def get_by_provider_by_id(self, provider_id):
        ...
        cursor.execute(
            '''SELECT id, provider_id, name, actual_model, model_type,
                      ...
                      created_at, last_checked, last_status, last_message, max_input_tokens
               FROM model
               WHERE provider_id = ?''',
            (provider_id,)
        )
    ...
    def get_by_provider(self, provider_name):
        ...
        cursor.execute(
            '''SELECT m.id, m.provider_id, m.name, m.actual_model, m.model_type,
                      ...
                      m.last_checked, m.last_status, m.last_message, m.max_input_tokens
               FROM model m
    ...
    def update(self, model_id, **kwargs):
        ...
        allowed_fields = ['name', 'actual_model', 'model_type', 'rpm_limit', 'tpm_limit',
                         ...
                         'timeout', 'stream_timeout', 'max_retries', 'supports_function_calling', 'skills', 'model_size_b', 'max_input_tokens']
        updates = []
        params = []
